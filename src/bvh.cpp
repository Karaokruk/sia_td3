...
inline const Eigen::Vector3f BVH::positionInFace(int face_id, int vertex_id) const
{
  return static_cast<Vector3f>(m_pMesh->position(pmp::Vertex(m_pMesh->faceIndices()(vertex_id,face_id))));
}
...
void BVH::build(const Mesh* pMesh, int targetCellSize, int maxDepth)
{
    ...
    m_pMesh = pMesh;
    m_nodes.resize(1);
    if(m_pMesh->n_faces() <= targetCellSize) {
        m_nodes[0].box = pMesh->boundingBox();
        m_nodes[0].first_face_id = 0;
        m_nodes[0].is_leaf = true;
        m_nodes[0].nb_faces = m_pMesh->n_faces();
        m_faces.resize(m_pMesh->n_faces());
        for(int i=0; i<m_pMesh->n_faces(); ++i)
        {
            m_faces[i] = i;
        }
    }else{
        m_nodes.reserve( std::min<int>(2<<maxDepth, std::log(m_pMesh->n_faces()/targetCellSize) ) );
        // compute centroids and initialize the face list
        m_centroids.resize(m_pMesh->n_faces());
        m_faces.resize(m_pMesh->n_faces());
        for(int i=0; i<m_pMesh->n_faces(); ++i)
        {
            m_centroids[i] = (positionInFace(i,0) + positionInFace(i,1) + positionInFace(i,2))/3.f;
            m_faces[i] = i;
        }
        buildNode(0, 0, m_pMesh->n_faces(), 0, targetCellSize, maxDepth);
    }
}
...
bool BVH::intersectNode(int nodeId, const Ray& ray, Hit& hit) const
{
    ...
    else
    {
        ...
        {
            found = intersectNode(child_id1, ray, hit);
        }
        if(tMin2 < hit.t() && tMin2<=tMax2 && tMax2>0)
        {
            found = found | intersectNode(child_id2, ray, hit);
        }
        return found;
    }
...
void BVH::buildNode(int nodeId, int start, int end, int level, int targetCellSize, int maxDepth)
{
    ...
    {
      aabb.extend(positionInFace(m_faces[i],0));
      aabb.extend(positionInFace(m_faces[i],1));
      aabb.extend(positionInFace(m_faces[i],2));
    }
    node.box = aabb;
    ...
    else if(m_splitMethod == SPLIT_SAH)
    {
        ...
        for(int i = start; i < end; ++i) {
            ...
            m_buckets[b].count++;
            m_buckets[b].bounds.extend(positionInFace(m_faces[i],0));
            m_buckets[b].bounds.extend(positionInFace(m_faces[i],1));
            m_buckets[b].bounds.extend(positionInFace(m_faces[i],2));
        }
