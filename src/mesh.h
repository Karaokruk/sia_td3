...
public:
  ...
  Eigen::Map<Eigen::Matrix3Xf> disp() {
    auto &disp = get_vertex_property<Vector3f>("v:disp").vector();
    return Eigen::Map<Eigen::Matrix3Xf>(disp[0].data(), 3, disp.size());
  }
